...
EBodyCoreKinect::EBodyCoreKinect() {
	_current_body_command = 0x00;
	memset(_current_body_data, 0x00, sizeof(unsigned char) * EROBOT_BODY_DATA_PACKET_SIZE);
...
void EBodyCoreKinect::getBodyCommandPacket(unsigned char *buffer) {

	int nCheckSumSize	= EROBOT_CHECKSUM_PACKET_SIZE;
	int nBodySize		= EROBOT_COMMON_HEADER_PACKET_SIZE + EROBOT_BODY_DATA_PACKET_SIZE + nCheckSumSize;

	memset(buffer, 0x00, nBodySize);

	unsigned char checksum = utility->getChecksumWithNoOffset(_current_body_data, EROBOT_BODY_DATA_PACKET_SIZE);
	...
	for(int i=0 ; i<EROBOT_BODY_DATA_PACKET_SIZE ; i++) {
		buffer[i+4] = _current_body_data[i];
	}
	buffer[EROBOT_BODY_DATA_PACKET_SIZE+EROBOT_COMMON_HEADER_PACKET_SIZE]	= checksum;
}

void EBodyCoreKinect::addBodyCommandPacket(unsigned char* buffer, const int startIndex) {
	...
	buffer[startIndex+EROBOT_BODY_DATA_PACKET_SIZE+EROBOT_COMMON_HEADER_PACKET_SIZE]	= checksum;
}


void EBodyCoreKinect::addGetPositionPacket(unsigned char* buffer, const int startIndex) {
...
int EBodyCoreKinect::parseAndSetBodyDataPacket(int dataPacketSize, unsigned char* dataPacket) {
...
int EBodyCoreKinect::parseAndSetBodyDataPacket(int dataPacketSize, unsigned char* dataPacket, const int startIndex) {
...
void EBodyCoreKinect::setCommandValue(short values) {
	_current_body_command = values;
}


void EBodyCoreKinect::setLeftValue(short *values) {
	...
	utility->intToByte(values[3],			_current_body_data+19);
	utility->intToByte(values[4],			_current_body_data+20);
	utility->intToByte(values[5],			_current_body_data+21);
}

void EBodyCoreKinect::setRightValue(short *values) {
	...
	utility->intToByte(values[3],			_current_body_data+16);
	utility->intToByte(values[4],			_current_body_data+17);
	utility->intToByte(values[5],			_current_body_data+18);
}

void EBodyCoreKinect::setHeadValue(unsigned char *values) {
...
//	utility->intToByte(speedTilt,			_current_pantilt_data+11);
	...
	_current_body_data[22] = values[10];
	_current_body_data[23] = values[11];
...
void EBodyCoreKinect::setHeadValueZero() {

	utility->intTo2Bytes(0,			_current_body_data+12);
	utility->intTo2Bytes(0,			_current_body_data+14);
	utility->intToByte(0,			_current_body_data+22);
	utility->intToByte(0,			_current_body_data+23);
}

bool EBodyCoreKinect::isBodyPacket(unsigned char* buffer, const int startIndex) {
...
void EBodyCoreKinect::getBodyPositons(short* positions) {
	for(int i=0; i<8; i++) {
...
bool EBodyCoreKinect::isBodyErrorPacket(unsigned char* buffer) {
	if( (buffer[0] != 0x01)
		|| (buffer[1] != 0xF8)
		|| (buffer[2] != 0xF8)
		|| (buffer[3] != 0x0E) ) {
...
int EBodyCoreKinect::parseAndSetBodyErrorDataPacket(int dataPacketSize, unsigned char* dataPacket, short errorCode[8]) {
...
bool EBodyCoreKinect::isBodyPositionPacket(unsigned char* buffer) {
	if( (buffer[0] != 0x01)
		|| (buffer[1] != 0xF8)
		|| (buffer[2] != 0xF8)
		|| (buffer[3] != 0xF8) ) {
...
int EBodyCoreKinect::parseAndSetBodyPositionDataPacket(int dataPacketSize, unsigned char* dataPacket, short angle[8]) {
