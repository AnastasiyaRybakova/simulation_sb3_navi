...
#include <silbot3_msgs/TTSSetProperties.h>
...
typedef struct
{
  ...
  private:
    ...
    void initDropWav(const string& wavfilename)  throw (Exception);
    bool makeWavFile(string outputFileName,  int NumData);
    bool makeWaveHeader( FILE *fp, int NumData, int SamplingRate, int Ch );
  ...
  public:
    ...
    TTSNode();
    virtual ~TTSNode();
    bool makeCB(silbot3_msgs::TTSMake::Request& request, silbot3_msgs::TTSMake::Response& response);
    void setProertiesCB(const silbot3_msgs::TTSSetProperties::ConstPtr& properties);
  };
}
